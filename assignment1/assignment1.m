...
%
% Answer: basically wipes everything, so it removes the variables inputted
% and resets the page
%
...
%
% Answer: aov = anova(factors, y). This function performs either a 1, 2, or
% N-way ANOVA and would be used when you need to compare the means of three
% or more groups to determine if there's statistically significant
...
%
% Answer: for loop, which executes a block of code a set number of times.
% and the while loop, which keeps executing a block of code until the
...
%
% Answer: NaN stands for "Not-a-Number", ex: 0/0 would result in NaN
%
...
originalWeightoz = originalWeightlbs * 16;
...
absoluteValues = abs(originalVec)
...
% Area of a trapezoid: A = (base1 + base2) * height / 2

base1 = 4; %base #1 length (m)
base2 = 7; %base #2 length (m)
height = 3; %perp distance btwn bases (m)
trapArea = (base1 + base2) * height / 2 %final area (m^2)
...
rev = input('Enter total revenues: ');
cost = input ('Enter total costs ');
...
disp('Hello World!')
disp(input('Tell me abt your day: ','s'))
